#!/bin/bash
# Copyright (c) 2013 The EPPO Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete EPPO database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no EPPO datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old EPPO datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected EPPO 0.7 datadir."
    ;;
  3)
    echo "Detected EPPO pre-0.8 datadir."
    ;;
  4)
    echo "Detected EPPO 0.8 datadir."
    ;;
esac
