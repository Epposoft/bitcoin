...
The RPM spec file provided here is for EPPO-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://EPPO.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[EPPO git repository](https://github.com/EPPO/EPPO) but are not part of
...
Currently only `Source10` is used, for the example `EPPO.conf` file.
...
in the EPPO git repository. At present only an SVG version of the EPPO
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the EPPO
...
needed when 0.12.1 is released, a proper fix is already in the EPPO git
master branch.
...
idea to build EPPO Core against a static version of that library compiled
...
The `EPPO.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `EPPO-server` package. At this point, most RPM based
...
The `EPPO-server` package should have SELinux support. How to properly do
...
EPPO-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/EPPO/). He has been
packaging EPPO for Fedora at least since 2012.
...
4. I renamed the `EPPO` package that contains the Qt GUI to `EPPO-core` as
that appears to be how the general population refers to it, in contrast to
`EPPO-xt` or `EPPO-classic`. I wanted to make sure the general population
