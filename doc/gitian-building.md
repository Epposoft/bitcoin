...
*Setup instructions for a Gitian build of EPPO Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the EPPO
...
- [Building EPPO Core](#building-EPPO-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-EPPO-host/README.md).
...
Clone the git repositories for EPPO and Gitian.
...
in the EPPO repository under 'Fetch and create inputs' to install sources which require
...
To build EPPO Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the EPPO repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/EPPO/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/EPPO/EPPO
...
For example:
```bash
URL=https://github.com/laanwj/EPPO.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit EPPO=${COMMIT} --url EPPO=${URL} ../EPPO/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit EPPO=${COMMIT} --url EPPO=${URL} ../EPPO/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit EPPO=${COMMIT} --url EPPO=${URL} ../EPPO/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the EPPO git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../EPPO/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/EPPO
SIGPATH=/some/root/path/EPPO-detached-sigs

./bin/gbuild --url EPPO=${BTCPATH},signature=${SIGPATH} ../EPPO/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/EPPO-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/EPPO-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/EPPO-osx-build.assert
```
...
[EPPO-core/gitian.sigs](https://github.com/EPPO-core/gitian.sigs/) repository, or if that's not possible create a pull
